...
func detectLangInProfiles(text string, options Options, langProfileList langProfileList) (Lang, float64) {
	...
	default:
		return calculateConfidence(langDistances)
	}
}

func calculateConfidence(langDistances []langDistance) (Lang, float64) {
	...
	score1 := langDist1.dist
	score2 := langDist2.dist
	...
	confidentRate := float64(12.0/float64(langDist1.numTrigrams)) + 0.05
...
func calculateDistance(langTrigrams []string, textTrigramsWeights map[string]int) (totalDist, numberTrigrams int) {
	for i, trigram := range langTrigrams {
		if textTrigramsWeights[trigram] > 0 {
			numberTrigrams++
